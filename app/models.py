...
class User(db.Model):
    ...
    # Optional profile data for UI previews (e.g., LinkedIn-like post composer)
    full_name = db.Column(db.String(200), nullable=True)
    profile_image_url = db.Column(db.String(2000), nullable=True)
    profile_source = db.Column(db.String(20), nullable=True)  # linkedin | google
...
class Article(db.Model):
    ...
    # Cached full-article extraction (best-effort; used for higher-quality generation)
    content_text = db.Column(Text, nullable=True)
    content_extracted_at = db.Column(DateTime(timezone=True), nullable=True)
    content_extractor = db.Column(db.String(50), nullable=True)
